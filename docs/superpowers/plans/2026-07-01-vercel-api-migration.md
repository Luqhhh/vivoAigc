...
**Architecture:** Vercel imports `apps/api` as the project Root Directory and uses zero-config Express detection for the default-exported app in `src/server.ts`. The Vercel Project API verified `defaultResourceConfig.fluid=true` and `resourceConfig.fluid=true`. Fluid Compute must remain enabled so the function duration exceeds the Lanxin provider's 60-second timeout and leaves headroom for fallback and serialization; if Fluid is disabled, the provider timeout and function runtime must be reconsidered. The server module retains local port listening outside Vercel. `vercel.json` is schema-only and deliberately omits a traditional `functions` mapping. The API uses Node 22.x, the repository pins pnpm 10.34.4, and commit `f301ebf` extends the provider timeout after production observed a 44.5-second response and an abort at the former exact timeout boundary. Live verification records health, real-provider, and local fallback evidence without persisting credentials.
