...
Vercel 从仓库 `Luqhhh/vivoAigc` 部署 API，项目 **Root Directory** 必须设置为 `apps/api`。Vercel 的 zero-config Express 支持会从默认导出 Express app 的 `src/server.ts` 自动检测入口，并默认使用 Fluid Compute；`apps/api/vercel.json` 只保留官方 schema，不配置传统 `functions` glob。`apps/api/package.json` 固定 Node.js `22.x`，仓库固定 pnpm `10.34.4`。Lanxin provider 自身对上游请求强制执行 45 秒超时。
