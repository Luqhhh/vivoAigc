...
| `LANXIN_API_URL` | real 必需 | `https://api.example.com/...` | 官方 Lanxin 上游的完整 HTTPS POST 地址。 |
| `LANXIN_APP_ID` | real 必需 | 服务端 Secret | 服务端竞赛配置；API 会校验其存在，但官方已实现契约不会把它传给上游。 |
| `LANXIN_APP_KEY` | real 必需 | 服务端 Secret | 仅 API 进程读取，用于 Bearer Authorization。 |
| `LANXIN_MODEL` | 可选 | `Doubao-Seed-2.0-mini` | 官方竞赛模型名称。 |
...
1. 只在 `apps/api/.env` 或部署平台服务端 Secret 中设置 `LLM_MODE=real` 和四项 `LANXIN_*` 变量。
2. 确保上游使用 HTTPS，并确认其契约与下节一致。
3. 重启 API，再执行同一分析请求。

结果解释：

- `source: "lanxin"`：gateway 请求成功，模型内容通过 JSON 解析与 Zod 结构校验。
- `source: "fallback"`：已进入 real 路径，但配置、网络、60 秒超时、HTTP 状态、响应 JSON、模型 JSON 或业务 schema 任一环节失败，后端明确返回稳定 mock 降级结果。
...
`LANXIN_API_URL` 指向的官方上游必须接受以下请求。`LANXIN_APP_ID` 是必需且会被服务端校验的竞赛配置，但当前已实现契约不会把它传给上游。
...
请求 header 不包含 `x-lanxin-app-id`，请求体也不包含 `appId` 字段。
...
适配器超时为 **60 秒**。生产观测曾出现 44.5 秒成功响应，并在原精确超时边界中止，因此 commit `f301ebf` 延长了 provider 超时。请求失败、非 2xx、响应不是 JSON、模型内容不是 JSON、结构校验失败或超时都会触发 `fallback`；原始上游错误和凭据不会返回前端。
...
Vercel 从仓库 `Luqhhh/vivoAigc` 部署 API，项目 **Root Directory** 必须设置为 `apps/api`。Vercel 的 zero-config Express 支持会从默认导出 Express app 的 `src/server.ts` 自动检测入口；本次通过 Vercel Project API 验证 `defaultResourceConfig.fluid=true` 且 `resourceConfig.fluid=true`。Fluid Compute 必须保持启用，使函数执行时长超过 Lanxin provider 的 60 秒超时，并为 fallback 和序列化保留余量；如果禁用 Fluid，必须重新评估 provider 超时和函数运行时长。`apps/api/vercel.json` 只保留官方 schema，不配置传统 `functions` glob。`apps/api/package.json` 固定 Node.js `22.x`，仓库固定 pnpm `10.34.4`。

以上配置修正来自 commit `5ed8d1d` 的真实 Vercel 生产构建错误；当前不表示修正后的生产重新部署已经成功，仍须按下文完成部署和线上验收。

在 Vercel Dashboard 中按以下顺序操作：

1. 选择 **Add New > Project**，导入 GitHub 仓库 `Luqhhh/vivoAigc`。
2. 在首次部署前展开项目配置，把 **Root Directory** 设置为 `apps/api`；Framework Preset 保留自动检测到的 Express。
3. 为 **Production** 环境添加以下变量：
   ...
   | `LANXIN_APP_ID` | 在 Dashboard 中输入真实敏感值 |
   | `LANXIN_APP_KEY` | 在 Dashboard 中输入真实敏感值 |

   `LANXIN_APP_ID` 和 `LANXIN_APP_KEY` 只允许作为 Vercel Dashboard 的敏感环境变量输入；不要把真实值写入本文、仓库、构建命令、日志、Web 变量或 APK。
4. 打开 **Settings > Environment Variables > Automatically expose System Environment Variables**，确认该选项已启用，使运行时暴露 `VERCEL=1`；新项目通常默认启用。当前入口据此只导出 Express app、不自行打开监听端口；Vercel 的 zero-config Node server 同时兼容打开 listener 的常见入口形式。
5. 点击 **Deploy**。
6. 部署后打开 **Settings > Environments > Production > Branch Tracking**，设置为 `codex/real-lanxin-apk`。该设置只影响后续部署，不会替换已经完成的生产部署。
7. 保存后打开 **Deployments > Create Deployment**，选择 `codex/real-lanxin-apk` 创建新部署；也可以在保存设置后向该分支推送一个新 commit 触发部署。

在执行健康检查前，先确认 Production Deployment 显示的来源分支是 `codex/real-lanxin-apk`，commit SHA 与预期交付 commit 一致。随后以 Dashboard 显示的 Production Domain 为准，预热并验证健康检查。不要把 Preview Deployment URL 当作 APK 的生产 API origin。

```powershell
$ApiOrigin = 'https://replace-with-production-domain.vercel.app'
$Health = Invoke-RestMethod -Method Get -Uri "$ApiOrigin/api/health"
$Health | Select-Object ok, llmMode | ConvertTo-Json
```

如果首次请求仍在冷启动，等待后重试，直到 HTTP 200 响应包含 `ok: true` 和 `llmMode: "real"`。这只证明 API 已启动且配置为 real 模式，不证明真实模型调用成功。

随后用真实 Fibonacci 请求验证 `/api/analyze-code`：

```powershell
$Body = @{
  language = 'python'
  code = "def fib(n):`n    if n <= 1:`n        return n`n    return fib(n - 1) + fib(n - 2)"
  stdin = "4`n"
  visualizationFocus = 'recursion'
  userLevel = 'beginner'
} | ConvertTo-Json

$Result = Invoke-RestMethod -Method Post -Uri "$ApiOrigin/api/analyze-code" -ContentType 'application/json; charset=utf-8' -Body $Body
$Result | Select-Object requestId, title, source, warnings | ConvertTo-Json -Depth 5
```

- 真实 Fibonacci 验收要求 `source: "lanxin"`。只有该值才证明本次结果来自 Lanxin。
- `source: "fallback"` 且 `warnings` 包含 `code: "MOCK_USED"` 和“蓝心真实服务暂不可用，已切换为稳定 mock 演示。”，表示 API 已进入 real 路径，但配置、网络、上游状态、超时或响应校验失败，并返回了稳定 mock 结果。这能维持演示，但不算真实分析通过；应检查 Vercel Function Logs 和环境变量后重试。
- 当前内存限流器按函数实例维护计数，不是跨实例的全局限流。生产配额或防滥用要求应在 Vercel 前置层、API gateway 或共享存储中实现，不能把单实例计数视为全局保护。

构建 APK 前，再次从 **Settings > Domains** 确认 Production Domain，使用该精确 HTTPS origin 验证 `/api/health` 和真实 Fibonacci 请求，然后把同一 origin 写入 APK 构建时的 `VITE_API_BASE_URL`。生产 origin 未确认或真实请求仍为 fallback 时，不要把对应生产联调标记为 PASS。
...
当前 CodeMotion API 已实现基于进程内存的单实例速率限制，但未实现 API 用户鉴权、跨实例全局速率限制、请求配额或最大并发控制。Vercel 上计数按函数实例维护，因此公开部署前必须在 Vercel 前置层、API gateway 或共享存储中落实：
...
$PatternFindings = @(
  Get-ChildItem -LiteralPath (Join-Path $ScanRoot 'unpacked') -Recurse -File -ErrorAction SilentlyContinue |
    Select-String -Pattern 'LANXIN_APP_KEY','LANXIN_APP_ID','Authorization\s*[:=]\s*Bearer' -ErrorAction SilentlyContinue
)
...
```powershell
$PatternCounts = @(git grep -c -I -E 'LANXIN_APP_KEY=.+|LANXIN_APP_ID=.+|Authorization.{0,20}Bearer' 2>$null)
$PatternCounts
...
`llmMode` 只表示配置。检查 gateway URL、服务端 Secret、上游 HTTP 状态和返回契约；适配器在 60 秒后超时，并会对模型 JSON 做严格 schema 校验。
