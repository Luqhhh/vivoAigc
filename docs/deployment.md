...
Vercel 从仓库 `Luqhhh/vivoAigc` 部署 API，项目 **Root Directory** 必须设置为 `apps/api`。Vercel 会自动检测 Express；`apps/api/package.json` 固定 Node.js `22.x`，仓库固定 pnpm `10.34.4`。`apps/api/vercel.json` 把 `src/server.ts` 的函数执行时限设置为 60 秒。
