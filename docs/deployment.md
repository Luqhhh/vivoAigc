...
### Render Blueprint 部署 API

本仓库根目录的 `render.yaml` 定义了一个免费的 Node Web Service `codemotion-api`，并固定使用 Node 22、官方 Lanxin 端点、`Doubao-Seed-2.0-mini`、real 模式和 Capacitor origin `https://localhost`。本任务只提交 Blueprint，不执行实际部署。

在 Render Dashboard 中按以下顺序操作：

1. 选择 **New > Blueprint**。
2. 连接 GitHub 仓库 `Luqhhh/vivoAigc`。
3. 选择最终交付分支 `codex/real-lanxin-apk`，不要误选其他开发分支或尚未包含 `render.yaml` 的分支。
4. 确认 Render 读取仓库根目录的 `render.yaml`，然后应用 Blueprint。
5. Dashboard 只会要求输入 `LANXIN_APP_ID` 和 `LANXIN_APP_KEY`；只在这两个 Secret 输入框中填写真实凭据。不要新增或覆盖 Blueprint 已声明的其他环境变量，也不要把凭据写入仓库、构建命令、日志、Web 变量或 APK。

部署完成后，以 Dashboard 显示的服务域名为准。服务名未被占用时，健康检查 URL 通常为 `https://codemotion-api.onrender.com/api/health`。HTTP 200 响应应包含 `ok: true` 和 `llmMode: "real"`；这只证明 API 已启动且配置为 real 模式，不证明真实模型调用成功。

随后发起一次 `/api/analyze-code` 请求并检查响应：

- 真实分析验收要求 `source: "lanxin"`。只有该值才证明本次结果来自 Lanxin。
- Render 免费服务冷启动时，第一次健康检查或业务请求可能需要等待，客户端也可能显示预热或重试状态。先等待健康 URL 返回 HTTP 200，再重试分析；冷启动期间的超时不能直接判定为 Lanxin 凭据或模型故障。
- `source: "fallback"` 且 `warnings` 包含 `code: "MOCK_USED"` 和“蓝心真实服务暂不可用，已切换为稳定 mock 演示。”，表示 API 已进入 real 路径，但配置、网络、上游状态、超时或响应校验失败，并返回了稳定 mock 结果。这可保证演示继续，但不算真实分析通过；应检查 Render 服务日志和 Secret 配置后重试。
